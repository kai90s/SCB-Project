...
if (isset($_POST['btn_submit_newpassword'])) {
  ...
  // update password on DB
    $qr=mysqli_query($db,"UPDATE user SET password ='$newpassword' WHERE u_email ='$email' ");
    if($qr==false){
      ...
      echo "<script>alert(\"Your password successfully changed. redirect to login menu\"); window.location = (\"login.php?success=passwordchange\")</script>";
